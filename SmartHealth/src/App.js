...
export default function App() {
  ...
  // Ambil data pasien dari Firebase
  useEffect(() => {
    ...
    onValue(patientsRef, (snapshot) => {
      ...
      if (data) {
        const list = Object.entries(data).map(([id, value]) => ({
          id,
          name: value.name || id,
          data: value.bpm_history ? Object.values(value.bpm_history) : [],
        }));
        setPatients(list);
      }
    });
  ...
  // Live update sensor dari pasien pertama (contoh)
  useEffect(() => {
    const sensorRef = ref(db, "/patients/P001/bpm_history");
    const unsubscribe = onValue(sensorRef, (snapshot) => {
      const data = snapshot.val();
      if (data) {
        setSensorData(Object.values(data));
      }
    });
  ...
  const renderHome = () => (
    ...
    >
      ...
      <Button className="mt-6" onClick={() => setPage("sensor")}>
        Get Started
      </Button>
    </div>
  );
